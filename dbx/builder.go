...
type Builder interface {
	// DriverName returns the name of the driver being used by this builder.
	DriverName() string
...
func (b *BaseBuilder) DriverName() string {
	if b.db != nil {
		return b.db.DriverName()
	}
	return ""
}
...
func (b *BaseBuilder) RenameTable(oldName, newName string) *Query {
	// default base rename behavior
	if b.db.DriverName() == "postgres" || b.db.DriverName() == "pgx" {
		sql := fmt.Sprintf("ALTER TABLE %v RENAME TO %v", b.db.QuoteTableName(oldName), b.db.QuoteSimpleTableName(strings.Trim(newName, "\"{}")))
		return b.NewQuery(sql)
	}
