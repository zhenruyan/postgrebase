...
func (b *PgsqlBuilder) RenameTable(oldName, newName string) *Query {
	// PostgreSQL's RENAME TO takes a simple identifier for the new name, not a fully qualified/quoted table name with schemas.
	// We extract just the table name part if necessary and quote it simply.
	if strings.Contains(newName, ".") {
		parts := strings.Split(newName, ".")
		newName = parts[len(parts)-1]
	}
	
	quotedOldName := b.db.QuoteTableName(oldName)
	// target name shouldn't have quotes/prefix in PostgreSQL's RENAME TO statement
	newNameRaw := strings.Trim(newName, "\"{}")
	quotedNewName := b.db.QuoteSimpleTableName(newNameRaw)
	
	sql := fmt.Sprintf("ALTER TABLE %v RENAME TO %v", quotedOldName, quotedNewName)
	return b.NewQuery(sql)
}
